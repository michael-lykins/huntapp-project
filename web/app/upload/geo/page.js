...
export default function Page(){
  ...
  async function onSubmit(e){ e.preventDefault(); setLoading(true); const form=new FormData(e.currentTarget);
    try{ const r=await fetch(`${API_BASE}/api/geo/upload`,{method:'POST',body:form}); const j=await r.json(); setResp(j); }catch(err){ setResp({ok:false,error:String(err)}) } finally{ setLoading(false) } }
