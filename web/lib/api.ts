...
export async function getNearbyEvents(args: {
  ...
  const p = new URLSearchParams({
    lat: String(args.lat),
    lon: String(args.lon),
    radius_m: String(args.radius_m),
    limit: String(args.limit ?? 50),
  });
  const res = await fetch(`${API_BASE}/events/near?${p.toString()}`, { cache: "no-store" });
  if (!res.ok) {
    const txt = await res.text().catch(() => "");
    throw new Error(`near failed: ${res.status} ${res.statusText} ${txt}`);
  }
